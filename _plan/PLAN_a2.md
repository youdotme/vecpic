...
| `vecpic -i a.png -o out.pdf --format svg` | 报错(冲突) |
...
> 内部把 11 个 vtracer 参数收拢到 `overrides` dict 后传给 `VtracerConfig.merged()`,避免业务代码再处理 `**kwargs`:
...
9. format=svg → os.replace() 原子移动到目标路径;
10. format=pdf/eps → writer.export_svg() 导出;keep_svg 决定是否保留中间 SVG;
11. 返回最终输出路径。
```
...
SVG 输出使用 `os.replace(tmp_svg, final_svg_path)`。

PDF/EPS 导出时:中间 SVG 默认清理;`--keep-svg` 时保留到同目录。
...
writer.validate_svg()
      │
      ├─ format=svg → os.replace(tmp_svg, final_svg)
      ├─ format=pdf → writer.export_svg(..., "pdf")
      └─ format=eps → writer.export_svg(..., "eps")
```
...
测试点:真实格式检测、扩展名不匹配 warning、CMYK→RGB、P/L/LA 转换、透明 alpha 保留、flatten 背景、max_size 缩放、EXIF 修正、多帧首帧、空文件、损坏文件、不支持格式。

### 8.3 converter 测试

使用 `monkeypatch` 注入假的 `vtracer` 模块。

测试点:vtracer 未安装抛 `VtracerNotInstalledError`;Pillow Image 转 RGBA;pixels 为 `bytes`;size 正确;配置参数正确传入;参数别名映射正确;vtracer 抛异常时包装为 `ConversionFailedError`;SVG 字符串正确写入文件;DEBUG 日志不包含像素数据。

### 8.4 writer 测试

测试点:有效 SVG 通过;非 XML 抛 `SvgValidationError`;根节点不是 SVG 抛错;缺尺寸信息抛错;缺图形元素抛错;嵌套 `<g><path /></g>` 通过;namespace 与无 namespace 均处理;PDF/EPS backend 缺失抛 `ExportBackendMissingError`;外部命令失败抛 `ExportFailedError`。

### 8.5 pipeline & CLI 测试

pipeline 关注:默认输出路径、显式输出路径、`--format` 推断、`--format` 与后缀冲突、preset 合并、显式参数覆盖 preset、临时文件清理、SVG 原子替换、PDF/EPS 中间 SVG 清理、`keep_svg=True` 保留中间 SVG。
...
测试点:`vecpic -h`、`python -m vecpic -h`、PNG → SVG、缺失输入文件、不支持格式、参数非法、verbosity、`--format` 冲突、`--preset`、`--flatten-bg`、`--max-size`。

### 8.6 回归测试(视觉 + 结构)

不作为 0.1.0 发布硬门槛,可后置。

视觉:SVG → CairoSVG/Inkscape 渲染回 PNG → 与原图对齐尺寸 → 计算 SSIM;阈值暂定 `>= 0.85`,照片更宽松,flat design 更严格。

结构:path 数量在合理区间;SVG 文件大小在合理范围;viewBox 比例匹配原图;SVG 解析正常;不出现空 SVG。

不使用 hash 比对 SVG:vtracer 不同版本输出 path 顺序、浮点精度、格式化均可能变化,hash 断言极脆弱。
...
平台:Linux / macOS / Windows;Python:3.10 / 3.11 / 3.12。

内容:config、reader、writer validate、converter 桩化、pipeline 桩化、CLI 基础测试、ruff、mypy。

### 9.2 vtracer 集成测试

优先 Linux,Python 3.10/3.11/3.12,测试 PNG → SVG、JPEG → SVG、透明 PNG → SVG。

```python
@pytest.mark.slow
```

本地:`pytest -m slow`。默认 CI 可跳过 slow,或仅在 Linux 跑 slow。
...
原则:这些能力均不应进入 0.1.0 核心依赖;新功能优先以可选依赖或可选 backend 形式接入。
